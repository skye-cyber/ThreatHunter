            ...
            for rule_name in files:
                ...
                try:
                    with open(rule_path, "rb") as f:
                        rule = f.read().decode("utf-8", errors="ignore")
                        # print(rule)
                except Exception as e:
                    logger.error(
                        f"Failed to read file {rule_path}: {e}",
                        exc_info=True,
                        stack_info=True,
                    )
                    continue
